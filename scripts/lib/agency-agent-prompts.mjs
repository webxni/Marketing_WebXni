...
const PLATFORM_QUALITY_STANDARD = [
  ...
  '- TikTok/Reels/video: action-oriented hook that matches a visual sequence the designer can execute. TikTok delivery is capped at 90 total characters, so a complete useful hook may omit a CTA or hashtags.',
  '- Pinterest: the delivery field is a title capped at 100 total characters. Judge it as concise discovery copy, not as a full description.',
].join('\n');
...
export function buildAgencyPrompt(kind, { client, snapshot, task }) {
  ...
  if (kind === 'orchestratorReview') {
    return `${shared}\n\nAct as the agency operations coordinator. Prioritize only current, evidence-backed work. Every finding must set state to active, recovered, or historical and list evidence_ids copied exactly from the supplied snapshot. Every entry under recent_generation_failures is a terminal incident record, not an active run; never use one alone to claim that a slot remains incomplete. Current package counts and later replacement content override an older partial-run count. Recovered/historical findings must use info severity and must not create assignments. waiting_marvin_approval and waiting_designer_assets are intentional human workflow gates, not failures or bottlenecks unless supplied evidence includes an overdue age or breached SLA. Convert each active actionable issue into an assignment naming the responsible existing agent, one concrete action, priority, and reason. Respect profile-completeness, editorial-review, Marvin approval, and designer gates. Do not duplicate reliability/security findings and do not claim an assignment was executed.`;
  }
